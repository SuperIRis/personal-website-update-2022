...
function onSendForm(e) {
	...
	if ($("#robot-checkbox").is(":checked")) {
		$("#robot-checkbox")
			.parent()
			.append(
				"<span class='error'>// Lo siento, los correos de robots sólo se reciben entre 4:00 y 4:01 AM</span>"
			);
		error++;
	}
		...
		sendMail(
			...
			function () {
				$("#c-form").fadeOut(function () {
					$("#c-form").replaceWith(
						"<div class='form-sent'>¡Listo! Mientras te contesto, un xkcd <p><img src='http://imgs.xkcd.com/comics/keyboard_problems.png' alt='' /></p></div>"
					);
				});
			}
		);
	}
	trackEvent("contact-form", "send", "errors:" + error);
}
